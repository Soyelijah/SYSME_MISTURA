...
$_POST['stock'] = str_replace(',', '.', $_POST['stock']);
	...
	// encuentra el post id de la variación
    $sql = 'select post_id from ' . $table_prefix . 'postmeta where meta_key = "_sku" and meta_value = "'.$codbarras.'" and post_id in (select post_parent from ' . $table_prefix . 'posts where post_id = '.$product_id.')';
	$result2 = mysqli_query($conexion, $sql);
	$row2 = mysqli_fetch_array($result2);
    $product_id = $row2['post_id'];
	
	//$errores = $errores.mysqli_num_rows($result2)."-".$product_id."-".$sql."-";
